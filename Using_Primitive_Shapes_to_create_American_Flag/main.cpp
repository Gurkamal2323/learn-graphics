...
void DrawStar(float fX, float fY) {
	...
	// Draw ten triangles
	const float kfRadius = 0.0616/2.0; //inner radius
	const float kfInnerRadius = kfRadius*(1.0/(sin((2.0*kfPi)/5.0)*2.0*cos(kfPi/10.0) + sin((3.0*kfPi)/10.0))); //outer radius
	glColor3f(1.0, 1.0, 1.0);
	glBegin(GL_TRIANGLE_FAN); //stars are drawn using triangle fan
...
void DrawStripes() {
    //loop will run 13 times one for each stripe
    //we are drawing stripes from bottom to top
	for (int iStripeIndex = 0; iStripeIndex < 13; ++iStripeIndex) {
		// Alternate stripe colors
		if (iStripeIndex % 2 == 0) {
			glColor3f(204.0/255.0, 0.0, 0.0); //red
		} else {
...
void Initialize() {
	glClearColor(0.0, 0.0, 102.0/255.0, 0.0); //(R,G,B,A), R=red, G=green, B=blue, A=alpha which is opacity range from 0 to 1
...
int main(int iArgc, char** cppArgv) {
	...
	glutCreateWindow("American Flag");
	Initialize();
